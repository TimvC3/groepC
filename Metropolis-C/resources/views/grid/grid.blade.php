<x-app-layout>
    @php
        $approvedFacilities = $groupedFacilities->flatten(1)->values();

        $approvedDestinations = [
            2 => $approvedFacilities->get(0),
            6 => $approvedFacilities->get(1),
        ];
    @endphp
                                ...
                                <section class="category-section min-w-56 flex-none xl:min-w-0">
                                    ...
                                    <div class="mt-3 flex gap-3 xl:grid xl:grid-cols-1">
                                        @foreach ($facilities as $facility)
                                            <div class="zoning-item w-44 flex-none cursor-grab active:cursor-grabbing rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-4 shadow-sm transition hover:border-indigo-400 xl:w-full"
                                                draggable="true"
                                                data-id="{{ $facility->id }}"
                ...
                <section class="xl:col-span-2">
                    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6 flex flex-col items-center">
                        <div class="flex justify-between items-center mb-6 w-full max-w-md">
                            ...
                            <button id="clear-grid" type="button"
                        ...
                        <div class="grid grid-cols-4 gap-1 justify-center border-4 border-gray-100 dark:border-gray-700 p-2 rounded-3xl dark:bg-gray-900/50">
                            @for ($i = 1; $i <= 12; $i++)
                                @php
                                    $approvedFacility = $approvedDestinations[$i] ?? null;
                                @endphp

                                <div
                                    class="grid-cell h-16 w-16 rounded-xl border-2 flex flex-col items-center justify-center text-center p-1 transition-all sm:h-24 sm:w-24
                                        {{ $approvedFacility
                                            ? 'border-solid border-green-500 bg-green-50 dark:bg-green-900/20 cursor-not-allowed'
                                            : 'border-dashed border-gray-300 dark:border-gray-600'
                                        }}"
                                    data-index="{{ $i }}"
                                    @if ($approvedFacility)
                                        data-approved="true"
                                        data-item-id="{{ $approvedFacility->id }}"
                                        data-item-type="facility"
                                        aria-label="{{ $approvedFacility->name }}"
                                        title="Approved destination: this cell can no longer be changed or removed."
                                        draggable="true"
                                    @endif
                                >
                                    @if ($approvedFacility)
                                        <div class="relative flex flex-col items-center">
                                            <div class="text-2xl pointer-events-none">
                                                {{ $approvedFacility->icon }}
                                            </div>

                                            <span class="mt-1 rounded-full border border-green-600 bg-green-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-green-700">
                                                Approved
                                            </span>
                                        </div>
                                    @else
                        ...
                        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6 flex flex-col">
                            <div class="flex justify-between items-center mb-6 w-full">
                                ...
                                <div class="text-right">
                                    ...
                                    <div id="effect-total-score" class="text-3xl font-bold text-gray-900 dark:text-gray-100">
                                        0
                                    </div>
                                </div>
                            </div>
